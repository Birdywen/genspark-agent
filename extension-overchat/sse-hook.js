// sse-hook.js — Overchat SSE interceptor (runs at document_start in MAIN world)
// Hooks fetch + WebSocket + EventSource to capture raw streaming data
// Uses defineProperty to survive bundle overwriting fetch
...
(function() {
  ...
  // ── Helper: check if URL is an Overchat chat completion POST ──
  function isConversationPost(url, method) {
    if (method && method.toUpperCase() !== 'POST') return false;
    return url.indexOf('api.overchat.ai') !== -1 &&
  ...
  function interceptStream(resp, url) {
    try {
      ...
      document.dispatchEvent(new CustomEvent('__sse_connected__', {
        detail: { transport: 'overchat-fetch', url: url, timestamp: Date.now() }
      }));
      ...
      function processLine(line) {
        ...
        if (line.startsWith('data: ')) {
          ...
          try {
            ...
            // Overchat uses standard OpenAI format: choices[0].delta.content
            if (parsed.choices && parsed.choices[0]) {
              var choice = parsed.choices[0];
              if (choice.delta && choice.delta.content) {
                document.dispatchEvent(new CustomEvent('__sse_data__', {
                  detail: {
                    data: JSON.stringify({ type: 'content_delta', text: choice.delta.content }),
                    timestamp: Date.now()
                  }
                }));
              }
        ...
        reader.read().then(function(chunk) {
          if (chunk.done) {
            ...
            document.dispatchEvent(new CustomEvent('__sse_closed__', {
              detail: { transport: 'overchat-fetch', timestamp: Date.now() }
            }));
            return;
          }
    ...
    try {
      Object.defineProperty(window, 'fetch', {
        ...
        set: function(newFetch) {
          // Overchat's bundle is overwriting fetch — wrap their version too
  ...
  console.log('[SSE-Hook] Overchat interceptor active (fetch/defineProperty + WS + ES)');
})();
