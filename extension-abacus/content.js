  ...
  function safeJsonParse(jsonStr) {
    ...
    try {
      ...
      try {
        ...
        const toolMatch = fixed.match(/"tool"\s*:\s*"([^"]+)"/);
        ...
        const modeMatch = fixed.match(/"mode"\s*:\s*"([^"]+)"/);
        const stepsMatch = fixed.match(/"steps"\s*:/);
        if (toolMatch) {
          const params = {};
          if (pathMatch) params.path = pathMatch[1];
          if (cmdMatch) params.command = cmdMatch[1];
          if (cmdLineMatch) params.command_line = cmdLineMatch[1];
          if (modeMatch) params.mode = modeMatch[1];
          console.warn('[Agent] Partial parse for tool:', toolMatch[1], params);
          return { tool: toolMatch[1], params, _partialParse: true };
        }
        if (stepsMatch) {
          console.warn('[Agent] Partial parse detected steps but JSON parse failed');
        }
        throw e1;
      }
    }
...
`;
  ...
  function extractBalancedJson(text, marker, fromEnd = false) {
    ...
    while (searchFrom < text.length) {
      let ocStart = text.indexOf(ocPrefix + String.fromCharCode(10), searchFrom);
      if (ocStart === -1) ocStart = text.indexOf(ocPrefix + "{", searchFrom);
      if (ocStart === -1) ocStart = text.indexOf(ocPrefix + " {", searchFrom);
      ...
      if (/Example:|e\.g\.|示例|格式/.test(beforeOC)) {
        searchFrom = ocStart + ocPrefix.length;
        continue;
      }
      ...
      const rawBlock = text.substring(ocStart, ocEndIdx + ocEndTag.length);
      ...
      try {
        ...
        console.warn("[Agent] ΩCODE parse failed:", e.message, rawBlock.slice(0, 300));
      }

      searchFrom = ocEndIdx + ocEndTag.length;
    }
