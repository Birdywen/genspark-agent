...
## Phase 5: 可执行命令库 (Executable Command Library)

### 概念
toolkit 槽位的 messages 通道作为云端命令库：
- 每条 message = 一个可执行 function(args)
- key = 命令名，content = JS function 代码 + 注释说明
- 执行验证后才入库，保证质量
- 支持传参，比静态命令灵活得多

### 需要实现
1. **vfs.execMsg(slot, key, args)** — 从 messages 通道读取代码并执行
   - 读 readMsg(slot, key) → new Function('args', code) → fn(args)
   - 支持 async（返回 Promise 自动 await）
2. **命令入库流程** — 执行成功的命令 → writeMsg 存入 toolkit
3. **命令列表** — listMsg('toolkit') 查看所有可用命令

### 示例命令库


toolkit messages: deploy-cf → function(args) { /* Cloudflare Pages 部署 / } backup-beast → function(args) { / 猛兽 CDP 备份 / } health-check → function(args) { / 全套健康检查 / } ssh-beast → function(args) { / SSH 猛兽执行命令 / } context-save → function(args) { / 保存 session 摘要 */ }
