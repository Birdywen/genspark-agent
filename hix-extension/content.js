// content.js v2.0.0 - Hix Agent Bridge - SSE+DOM双通道 - ΩCODE统一通道 - Agent 心跳机制
(function() { console.log('=== HIX AGENT v2.0.0 (SSE+DOM) LOADED ===');
  'use strict';
...
function log(...args) {
  ...
  function initSSEListener() {
    ...
    log('SSE listener initialized (Hix mode)');
  ...
  function isAIGenerating() {
    // Hix: send button becomes disabled while AI is generating.
    try {
      const editor = document.querySelector('div[contenteditable="true"]');
      if (!editor) return false;
      let composer = editor;
      for (let i = 0; i < 8 && composer; i++) composer = composer.parentElement;
      const root = composer || document;
      const btns = root.querySelectorAll('button');
      let roundBtn = null;
      for (const b of btns) {
        const cls = b.className || '';
        if (cls.indexOf('size-[32px]') !== -1 && cls.indexOf('rounded-') !== -1) {
          roundBtn = b; break;
        }
      }
      if (!roundBtn) return false;
      const editorText = (editor.innerText || '').trim();
      // Editor empty + button disabled => check for streaming cursor in last AI msg
      if (!editorText && roundBtn.disabled) {
        const lastMd = document.querySelectorAll('.hix-chat-markdown-body');
        if (lastMd.length) {
          const last = lastMd[lastMd.length - 1];
          if (last.querySelector('[class*="cursor"],[class*="blinking"],.animate-pulse')) return true;
        }
        // Also: if a recent SSE delta arrived but no [DONE], we're streaming.
        if (window.__hixStreaming === true) return true;
        return false;
      }
      return false;
...
`;
  ...
  function getAIMessages() {
    // Hix: walk up from each markdown-body to find the message-row container
    // (`flex flex-col gap-x-2 gap-y-1.5 pb-6` or the surrounding flex).
    // User msg has `justify-end` on a child; AI msg doesn't.
    // Dedup: outer + inner markdown-body both match — keep only outer-most ones.
    const all = Array.from(document.querySelectorAll('.hix-chat-markdown-body'));
    const outermost = all.filter(el => !el.parentElement || !el.parentElement.closest('.hix-chat-markdown-body'));
    return outermost.filter(el => {
      let p = el;
      for (let i = 0; i < 8 && p; i++) {
        p = p.parentElement;
        if (!p) break;
        const cls = p.className || '';
        // Row container reached — stop walking.
        if (cls.indexOf('flex flex-col gap-x-2 gap-y-1.5') !== -1) {
          // Look for justify-end inside this row → user message.
          return p.querySelector('.justify-end') === null;
        }
      }
      // Fallback: treat as AI if no justify-end ancestor seen.
      return true;
    });
  }
  ...
  function sendMessage(text) {
    ...
    // Step 1: clear via select-all + deleteContentBackward.
    try {
    ...
    // Step 2: insertText (Slate processes via onBeforeInput -> eB.insertText).
    setTimeout(() => {
      input.dispatchEvent(new InputEvent('beforeinput', {
    ...
    // Step 3 (preferred): press Enter — Slate listens via onKeyDown and Hix's
    // composer treats it as send. Doesn't depend on send-button enable state.
    const pressEnter = () => {
      const ed = getInputBox();
      if (!ed) return false;
      ed.focus();
    ...
    // Step 3b (fallback): click the round send button if Enter doesn't clear input.
    const clickSendBtn = () => {
      const ed = getInputBox();
      if (!ed) return false;
      let p = ed, composer = null;
      for (let i = 0; i < 8; i++) { if (!p.parentElement) break; p = p.parentElement; composer = p; }
      const root = composer || document.body;
      for (const b of root.querySelectorAll('button')) {
        const cls = b.className || '';
        if (cls.indexOf('size-[32px]') !== -1 &&
            cls.indexOf('rounded-') !== -1 &&
            !b.disabled &&
            cls.indexOf('cursor-not-allowed') === -1) {
          b.click();
          return true;
        }
      }
    ...
    // Fire Enter ~700ms after insertText settles.
    setTimeout(() => {
      pressEnter();
      addLog('📤 Enter 发送', 'info');
      // Verify: if input still has the text after 1.5s, fall back to button.
      setTimeout(() => {
        const ed = getInputBox();
        const remaining = ed ? (ed.innerText || '').trim() : '';
        if (remaining && remaining.length > 3 && remaining !== 'Type your question here') {
          if (clickSendBtn()) addLog('📤 Enter 没生效,点按钮兜底', 'info');
          else addLog('⚠️ Enter+按钮都失败', 'warn');
        }
      }, 1500);
    }, 700);
