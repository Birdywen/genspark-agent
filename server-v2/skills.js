...
class SkillsManager {
  ...
  _loadLessons() {
    ...
    try {
      const rows = execSync(
        `sqlite3 -json "${KNOWLEDGE_DB}" "SELECT category, title, problem, solution FROM lessons_learned WHERE integrated = 0 AND category IN ('eval_js','ffmpeg','system') ORDER BY category, id"`,
