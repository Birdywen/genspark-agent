...
function buildJsCode(tool, params) {
  if (tool === 'vfs_write') {
    if (!params.slot) return { error: 'vfs_write 需要 slot 参数' };
    if (params.key) {
  ...
  if (tool === 'vfs_read') {
    if (!params.slot) return { error: 'vfs_read 需要 slot 参数' };
    if (params.key) {
  ...
  if (tool === 'vfs_delete') {
    if (!params.slot || !params.key) return { error: 'vfs_delete 需要 slot 和 key 参数' };
  ...
  if (tool === 'vfs_query') {
    if (!params.slot) return { error: 'vfs_query 需要 slot 参数' };
    const qOpts = {};
