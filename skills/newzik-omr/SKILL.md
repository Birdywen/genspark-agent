...
### 2. 完整流程（上传 → OMR → 下载 → 转 ABC）
```bash
node newzik-manager.js auto ./songs    # 全自动
# 或分步:
node newzik-manager.js upload ./songs  # 上传 PDF
...
### 3. Token 过期处理
API 返回 XML 错误时，用 refresh_token 刷新：
```bash
...
```
更新 `~/.agent_secrets` 中的 `NEWZIK_ACCESS_TOKEN` 和 `NEWZIK_REFRESH_TOKEN`。
...
修正方法：
1. 读取 .abc 文件（纯文本，几十行）
2. 对照 PDF 原谱（转 PNG: `magick -density 200 input.pdf output_%02d.png`）
3. 直接编辑 .abc 文本：和弦用 `"Am"` 标注，音符用 ABC 记谱法
4. 保存即完成
...
| `upload <dir>` | 上传目录中的 PDF |
| `submit [n]` | 提交 n 个 OMR 任务 |
| `download` | 下载已完成的 MusicXML |
| `delete <pattern>` | 删除匹配的曲目 |
| `cleanup` | 删除重复曲目 |
| `trash` | 查看回收站 |
| `purge` | 清空回收站 |
| `auto [dir]` | 自动完成全流程 |
...
- 已完成: 28 首（26 + Blue Danube Full + Secret Garden Passacaglia）
- 自动修复: 209 处（平均每首 6.7 处）
- ABC 格式: 平均 ~45 行/首（vs MusicXML ~500 行/首，节省 90% token）
